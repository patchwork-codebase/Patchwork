...
export function ActiveBuildCard({ room, isObserver, handleArchiveRoom, archivingId }: { room: Room; isObserver: boolean; handleArchiveRoom?: (id: string) => void; archivingId?: string | null }) {
  ...
  return (
    <div className="bg-transparent border border-slate-800 rounded-[20px] sm:rounded-[24px] p-4 sm:p-6 flex flex-col relative shadow-sm group hover:border-amber-500/30 hover:shadow-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500" tabIndex={0}>
      <div className="flex justify-between items-start mb-4">
        <div className="flex flex-wrap gap-2 w-full pr-8">
          ...
          <h3 className="m-0 text-[16px] sm:text-[18px] font-extrabold text-slate-100 font-display line-clamp-2 leading-snug break-words">{room.title}</h3>
          ...
          <div className="flex items-center gap-2 mt-1 w-full">
            ...
            <span className="text-[12px] font-bold text-slate-300">{room.builderName}</span>
            {room.builderIsVerifiedExpert && <VerifiedTick isVerified className="w-3.5 h-3.5 -ml-1" />}
            {room.builderOrgName && (
               <>
                 <span className="text-[10px] text-slate-400">@</span>
                 <span className="text-[11px] font-bold text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">{room.builderOrgName}</span>
               </>
            )}
          </div>
        </div>
        ...
        {isObserver && (
          <button 
            className="absolute top-4 right-4 sm:top-6 sm:right-6 text-slate-400 hover:text-amber-500 transition-colors bg-transparent rounded-full p-1"
      ...
      <div className="grid grid-cols-1 gap-3 mb-5 relative md:grid-cols-2">
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 flex flex-col justify-center shadow-inner">
      ...
      {room.updatedAt && (
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 mb-5 relative">
      ...
      <div className="flex flex-row gap-2 relative mt-auto">
        ...
        <Link to={`/dashboard/room/${room.id}`}
          ...
          className="inline-flex items-center justify-center w-11 h-11 bg-transparent hover:bg-slate-800/50 border border-slate-700 text-slate-600 hover:text-slate-100 rounded-xl active:scale-95 transition-all shadow-sm focus-ring">
  ...
  return (
    <div className="bg-transparent border border-emerald-500/20 rounded-[20px] sm:rounded-[24px] p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 relative overflow-hidden shadow-sm group hover:border-emerald-500/40 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500" tabIndex={0}>
      ...
      <div className="flex justify-between items-start relative mb-1">
        <div className="flex gap-3 sm:gap-4 w-full">
          <div className="flex-1 min-w-0">
            <h3 className="m-0 text-[15px] sm:text-[16px] font-extrabold text-slate-100 font-display line-clamp-2 break-words mb-1.5">{log.title}</h3>
            <div className="flex items-center gap-2 mb-2">
              <div className="w-5 h-5 rounded-full object-cover border border-slate-200 overflow-hidden shrink-0 relative">
                <UserAvatar userId={log.builderId} name={log.builderName} avatarUrl={log.builderAvatarUrl} />
              </div>
              <span className="text-[12px] font-bold text-slate-300">{log.builderName}</span>
              {log.projectStage && (
                <>
                  <span className="text-slate-300">·</span>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3 relative">
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl py-2 px-2 text-center flex flex-col justify-center shadow-inner">
          <div className="text-[15px] sm:text-[16px] font-black text-slate-100 font-display leading-none">{log.updateCount}</div>
          <div className="text-[8px] sm:text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">Updates</div>
        </div>
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl py-2 px-2 text-center flex flex-col justify-center shadow-inner">
          <div className="text-[15px] sm:text-[16px] font-black text-slate-100 font-display leading-none">{getObserverCount(log)}</div>
          <div className="text-[8px] sm:text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">Observers</div>
        </div>
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl py-2 px-2 flex items-center justify-center shadow-inner group-hover:bg-emerald-50 transition-colors">
...
export function CompletedBuildCard({ log }: { log: Room }) {
  ...
  return (
    <div className="bg-transparent border border-slate-800 rounded-[20px] sm:rounded-[24px] p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 relative overflow-hidden shadow-sm group hover:border-primary-400/30 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400" tabIndex={0}>
      <div className="flex justify-between items-start relative mb-1">
        <div className="flex gap-3 sm:gap-4 w-full">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="m-0 text-[15px] sm:text-[16px] font-extrabold text-slate-100 font-display line-clamp-2 break-words mb-1">{log.title}</h3>
            <div className="flex items-center gap-2 mb-1.5">
              <div className="w-4 h-4 rounded-full object-cover border border-slate-200 overflow-hidden shrink-0 relative">
                <UserAvatar userId={log.builderId} name={log.builderName} avatarUrl={log.builderAvatarUrl} />
              </div>
              <span className="text-[11px] font-bold text-slate-300">{log.builderName}</span>
            </div>
      ...
      {log.retrospectiveNote && (
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-3 sm:p-4 text-[12px] sm:text-[13px] text-slate-600 italic border-l-[3px] border-l-primary-400 shadow-sm">
          "{log.retrospectiveNote}"
        </div>
      )}
      
      <div className="flex flex-wrap gap-2 mt-auto">
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-500">
          <Clock size={12} /> {daysActive} days
        </div>
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-500">
          <Sparkles size={12} /> {log.updateCount} updates
        </div>
        <Link to={`/dashboard/room/${log.id}`} className="ml-auto flex items-center justify-center p-2 bg-slate-800 hover:bg-slate-100 border border-slate-200 text-slate-600 rounded-lg transition-colors focus-ring" aria-label="Open completed room">
