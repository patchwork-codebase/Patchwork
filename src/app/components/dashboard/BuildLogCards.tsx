...
export function ActiveBuildCard({ room, isObserver, handleArchiveRoom, archivingId }: { room: Room; isObserver: boolean; handleArchiveRoom?: (id: string) => void; archivingId?: string | null }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[20px] sm:rounded-[24px] p-4 sm:p-6 flex flex-col relative shadow-sm group hover:border-amber-500/30 hover:shadow-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500" tabIndex={0}>
      <div className="flex justify-between items-start mb-4">
        ...
        {isObserver && (
          <button 
            className="absolute top-4 right-4 sm:top-6 sm:right-6 text-slate-400 hover:text-amber-500 transition-colors bg-white rounded-full p-1"
...
export function ShippedBuildCard({ log }: { log: Room }) {
  return (
    <div className="bg-white border border-emerald-500/20 rounded-[20px] sm:rounded-[24px] p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 relative overflow-hidden shadow-sm group hover:border-emerald-500/40 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500" tabIndex={0}>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3 relative">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl py-2 px-2 text-center flex flex-col justify-center shadow-inner">
          <div className="text-[15px] sm:text-[16px] font-black text-slate-900 font-display leading-none">{getObserverCount(log)}</div>
          <div className="text-[8px] sm:text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">Observers</div>
        </div>
        <div className="bg-slate-50 border border-slate-200 rounded-xl py-2 px-2 flex items-center justify-center shadow-inner group-hover:bg-emerald-50 transition-colors">
...
export function CompletedBuildCard({ log }: { log: Room }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[20px] sm:rounded-[24px] p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 relative overflow-hidden shadow-sm group hover:border-primary-400/30 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400" tabIndex={0}>
      <div className="flex justify-between items-start relative mb-1">
        <div className="flex gap-3 sm:gap-4 w-full">
      ...
      {log.retrospectiveNote && (
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 sm:p-4 text-[12px] sm:text-[13px] text-slate-600 italic border-l-[3px] border-l-primary-400 shadow-sm">
          "{log.retrospectiveNote}"
        </div>
      )}
      
      <div className="flex flex-wrap gap-2 mt-auto">
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-500">
          <Clock size={12} /> {daysActive} days
        </div>
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-500">
          <Sparkles size={12} /> {log.updateCount} updates
        </div>
        <Link to={`/dashboard/room/${log.id}`} className="ml-auto flex items-center justify-center p-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 rounded-lg transition-colors focus-ring" aria-label="Open completed room">
