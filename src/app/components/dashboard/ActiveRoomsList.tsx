                ...
                >
                  ...
                  <div className="flex flex-col gap-2 w-full relative">
                    ...
                    <div className="flex items-start gap-2 min-w-0">
                      ...
                      <div className={`text-[14px] sm:text-[15px] font-extrabold transition-colors font-display leading-snug line-clamp-3 group-hover:underline break-words ${selectedRoomId === room.id ? 'text-slate-900' : 'text-slate-700 group-hover:text-[#8B7CF8]'}`}>
                        {room.title}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 text-[11px] sm:text-[12px] text-slate-400 font-mono font-medium min-w-0">
                        ...
                        <span>Day {Math.max(1, Math.floor((Date.now() - new Date(room.createdAt || room.created_at || Date.now()).getTime()) / (1000 * 60 * 60 * 24)) + 1)}</span>
                        <span className="text-slate-300">·</span>
