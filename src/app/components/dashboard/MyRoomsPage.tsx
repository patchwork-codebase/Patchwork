    ...
    <div className="max-w-[1000px] mx-auto w-full p-4 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-[28px] sm:text-[32px] font-extrabold text-slate-900 font-display tracking-tight mb-2 flex items-center gap-2">
            {isObserver ? (
              <><Eye className="w-7 h-7 text-purple-400" /> Following</>
            ) : (
              'My Rooms'
            )}
          </h1>
      ...
      {isLoading ? (
        <div className="flex flex-col gap-4">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="bg-white border border-slate-200 rounded-[24px] py-6 px-6 flex flex-col gap-4 animate-pulse">
        ...
        <div className="bg-white border border-slate-200 rounded-[32px] p-12 flex flex-col items-center justify-center text-center relative overflow-hidden shadow-sm mt-8">
          ...
          <div className="w-16 h-16 rounded-3xl bg-slate-50 border border-slate-200 flex items-center justify-center mb-6 text-primary-400 shadow-sm">
            {isObserver ? <Eye className="w-8 h-8 text-purple-400" /> : <FolderGit2 className="w-8 h-8" />}
          </div>
          <h3 className="text-slate-900 text-[20px] font-bold mb-2">
            {isObserver ? 'Not following any rooms yet' : 'No Build Rooms Yet'}
          </h3>
          <p className="text-slate-500 text-[15px] max-w-[360px] leading-relaxed mb-8">
            {isObserver
                  ...
                  <div className="flex flex-col gap-3 relative">
                    ...
                    <div className="flex flex-wrap items-center gap-2 pl-5">
                      ...
                      {isObserver && room.builderName && (
                        <span className="text-[11px] text-slate-500 font-medium">by {room.builderName}</span>
                      )}
                    </div>
