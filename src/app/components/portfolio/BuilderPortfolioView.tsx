...
export function BuilderPortfolioView() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f17] text-slate-100 font-sans antialiased selection:bg-primary-500/30 selection:text-white">
      {/* Top Navbar Header */}
      <header className="sticky top-0 z-50 border-b border-slate-800/80 bg-[#0b0f17]/90 backdrop-blur-md px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div onClick={() => navigate("/")} className="flex items-center gap-2 cursor-pointer group">
            ...
            <span className="font-extrabold text-base sm:text-lg tracking-tight text-white group-hover:text-primary-400 transition-colors">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-3.5 py-1.5 rounded-full border border-slate-700 bg-slate-900/80 text-slate-300 text-xs font-semibold hover:bg-slate-800 hover:text-white transition-all flex items-center gap-1.5 cursor-pointer"
            >
      ...
      {/* Main Profile Header */}
      <section className="relative border-b border-slate-800/60 bg-gradient-to-b from-[#111622] to-[#0b0f17] py-12 px-6 overflow-hidden">
        {/* Glow backdrop */}
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-start gap-8 relative z-10">
          ...
          <div className="relative shrink-0">
            <UserAvatar
              ...
              className="w-28 h-28 sm:w-32 sm:h-32 rounded-3xl border-2 border-slate-700/80 shadow-2xl object-cover ring-4 ring-slate-900"
            />
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-slate-300 font-mono text-sm sm:text-base font-semibold">
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs font-semibold">
              <div className="flex items-center gap-2 bg-slate-900/90 border border-slate-800 px-3.5 py-2 rounded-2xl text-amber-300 shadow-sm">
                <Flame className="w-4 h-4 text-amber-400 shrink-0" />
                <span>{rooms.length} Active Build Rooms</span>
              </div>

              <div className="flex items-center gap-2 bg-slate-900/90 border border-slate-800 px-3.5 py-2 rounded-2xl text-indigo-300 shadow-sm">
                <Activity className="w-4 h-4 text-indigo-400 shrink-0" />
                <span>{topProofs.length} Verified Proofs</span>
              </div>

              <div className="flex items-center gap-2 bg-slate-900/90 border border-slate-800 px-3.5 py-2 rounded-2xl text-emerald-300 shadow-sm">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {rooms.map((room) => (
                <div
                  ...
                  className="bg-[#111622] rounded-3xl p-6 border border-slate-800/80 hover:border-primary-500/50 shadow-lg hover:shadow-primary-500/5 transition-all cursor-pointer group flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between gap-3 mb-3">
                      <h3 className="font-bold text-white text-lg group-hover:text-primary-400 transition-colors flex items-center gap-2">
                        {room.title}
                  ...
                  <div className="pt-4 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
                    <span className="font-semibold text-slate-300">Phase: Private Beta</span>
          ...
          <section>
            ...
            <div className="space-y-4">
              {topProofs.map((update) => (
                <div
                  key={update.id}
                  className="bg-[#111622] rounded-3xl p-6 border border-slate-800/80 shadow-md hover:border-slate-700 transition-all"
                >
                  <div className="flex items-center justify-between gap-2 mb-3 text-xs text-slate-400">
                    <div className="flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-primary-400" />
                      <span className="font-bold text-white">{update.rooms?.title || "Build Room"}</span>
                    </div>
                    <span className="font-mono text-slate-500">{new Date(update.created_at).toLocaleDateString()}</span>
                  </div>

                  <p className="text-slate-200 text-sm sm:text-base leading-relaxed font-normal mb-4 whitespace-pre-wrap">
                    {update.content}
                  </p>
        ...
        {activeTab === "roadmap" && (
          <section>
            ...
            <div className="bg-[#111622] rounded-3xl p-6 border border-slate-800/80 shadow-md">
