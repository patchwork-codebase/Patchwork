    ...
    <div className="mb-8 bg-gradient-to-br from-primary-500/10 to-primary-400/5 border border-primary-500/20 rounded-[24px] p-6">
      ...
      {profile.expertDomains?.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-5">
          {(profile.expertDomains as string[]).map((d: string) => (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          { icon: Star, label: "Review score", value: profile.expertReviewScore ? `${profile.expertReviewScore}/5.0` : "—" },
          { icon: CheckCircle, label: "Reviews done", value: profile.expertReviewsCompleted || 0 },
          { icon: TrendingUp, label: "Acceptance rate", value: profile.expertAcceptanceRate ? `${profile.expertAcceptanceRate}%` : "—" },
          { icon: Clock, label: "Avg. response", value: profile.expertAvgResponseHours ? `${profile.expertAvgResponseHours}h` : "—" },
          { icon: ShieldCheck, label: "Open slots", value: profile.expertOpenSlots ?? 3 },
