    ...
    <>
      ...
      <div className="relative pt-6 px-4 pb-8 sm:px-8 sm:pb-10 text-center">
        ...
        {profile?.isVerifiedExpert && (
            <div className="absolute top-5 right-5 bg-black/40 backdrop-blur-md border border-slate-100 dark:border-white/10 px-3.5 py-1.5 rounded-full flex items-center gap-1.5 text-[12px] font-bold text-primary-400 shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
        ...
        {/* Name & Role */}
        <h1 className="text-[26px] sm:text-[30px] font-extrabold text-slate-900 dark:text-white tracking-tight leading-tight mt-5 font-display">
          {profile?.name}
        </h1>
        <div className="flex items-center justify-center gap-2 mt-1.5 mb-5 text-slate-500 dark:text-slate-400 font-medium text-[14px]">
          <span className="font-bold text-slate-900 dark:text-white">Patchwork</span> <VerifiedTick isVerified={true} className="w-4 h-4 text-primary-400" /> <span className="text-slate-600">•</span> <span className="capitalize">{profile?.role || 'Member'}</span>
        </div>

        {/* Bio */}
        <p className="text-slate-600 dark:text-slate-300 text-[15px] leading-relaxed max-w-sm sm:max-w-md mx-auto mb-7 font-medium">
          {profile?.bio || "Building better products that solve real problems and create meaningful impact."}
        </p>

        {/* Stats Pill */}
        <div className="flex justify-between items-center bg-white/5 backdrop-blur-sm rounded-full p-2.5 sm:p-3 max-w-sm mx-auto border border-slate-100 dark:border-white/10 mb-8 shadow-md">
          <div 
          ...
          >
            <div className="flex items-center gap-1.5 text-slate-900 dark:text-white font-extrabold text-[16px] sm:text-[18px] mb-0.5"><Users className="w-4 h-4 text-primary-400"/> {profile?.followerCount || 0}</div>
            <span className="text-[11px] sm:text-[12px] text-slate-500 dark:text-slate-400 font-medium">Followers</span>
          ...
          >
            <div className="flex items-center gap-1.5 text-slate-900 dark:text-white font-extrabold text-[16px] sm:text-[18px] mb-0.5"><Users className="w-4 h-4 text-primary-400"/> {profile?.followingCount || 0}</div>
            <span className="text-[11px] sm:text-[12px] text-slate-500 dark:text-slate-400 font-medium">Following</span>
            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-px h-8 bg-white/10" />
          </div>
          <div className="flex flex-col items-center flex-1 py-1">
            <div className="flex items-center gap-1.5 text-slate-900 dark:text-white font-extrabold text-[16px] sm:text-[18px] mb-0.5"><Trophy className="w-4 h-4 text-primary-400"/> {roomsCount}</div>
            <span className="text-[11px] sm:text-[12px] text-slate-500 dark:text-slate-400 font-medium">Projects</span>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center">
            <div className="flex justify-center gap-3">
              <div className="w-10 h-10 rounded-full border border-slate-100 dark:border-white/10 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-white/10 transition-colors cursor-pointer shadow-sm">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                   <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
              </div>
              {profile?.linkedinUrl && (
                <a href={profile.linkedinUrl} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-100 dark:border-white/10 flex items-center justify-center text-[#0A66C2] hover:bg-white/10 transition-colors shadow-sm">
                  <Linkedin className="w-4 h-4" fill="currentColor" />
                </a>
              )}
              {profile?.twitter && (
                <a href={`https://twitter.com/${profile.twitter.replace('@', '')}`} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-100 dark:border-white/10 flex items-center justify-center text-[#1DA1F2] hover:bg-white/10 transition-colors shadow-sm">
                  <Twitter className="w-4 h-4" fill="currentColor" />
                </a>
              )}
              {profile?.githubUrl && (
                <a href={profile.githubUrl} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-100 dark:border-white/10 flex items-center justify-center text-slate-900 dark:text-white hover:bg-white/10 transition-colors shadow-sm">
                  <Github className="w-4 h-4" fill="currentColor" />
                </a>
              )}
              {profile?.website && (
                <a href={profile.website} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-100 dark:border-white/10 flex items-center justify-center text-primary-400 hover:bg-white/10 transition-colors shadow-sm">
