  ...
  return (
    <>
      ...
      <div className="relative pt-6 px-4 pb-8 sm:px-8 sm:pb-10 text-center">
        ...
        <div className="absolute top-0 left-0 right-0 h-[180px] sm:h-[220px] rounded-t-[24px] sm:rounded-t-[32px] overflow-hidden -z-10 bg-gradient-to-br from-[#EAE5FF] via-[#D8D2FF] to-[#C4BDFF]">
          <img
          ...
          <svg className="absolute bottom-0 left-0 w-full" viewBox="0 0 1440 120" fill="none" preserveAspectRatio="none">
            <path d="M0,0 C320,100 420,-40 720,20 C1020,80 1120,0 1440,60 L1440,120 L0,120 Z" fill="white"/>
          </svg>
        </div>

        {/* Verified Expert Tag top right inside banner */}
        {profile?.isVerifiedExpert && (
            <div className="absolute top-5 right-5 bg-white/70 backdrop-blur-md px-3.5 py-1.5 rounded-full flex items-center gap-1.5 text-[12px] font-bold text-primary-700 shadow-[0_2px_10px_rgba(0,0,0,0.05)]">
        ...
        <div className="relative mx-auto mt-4 sm:mt-8 group w-fit">
          <div className="w-[100px] h-[100px] sm:w-[120px] sm:h-[120px] rounded-full ring-[4px] sm:ring-[6px] ring-white shadow-sm overflow-hidden relative bg-white">
            <UserAvatar 
              userId={profile?.id} 
              name={profile?.name} 
        ...
        {/* Name & Role */}
        <h1 className="text-[26px] sm:text-[30px] font-extrabold text-slate-900 tracking-tight leading-tight mt-5 font-display">
          {profile?.name}
        </h1>
        <div className="flex items-center justify-center gap-2 mt-1.5 mb-5 text-slate-600 font-medium text-[14px]">
          <span className="font-bold text-slate-700">Patchwork</span> <VerifiedTick isVerified={true} className="w-4 h-4 text-primary-500" /> <span className="text-slate-300">•</span> <span className="capitalize">{profile?.role || 'Member'}</span>
        </div>

        {/* Bio */}
        <p className="text-slate-500 text-[15px] leading-relaxed max-w-sm sm:max-w-md mx-auto mb-7 font-medium">
        ...
        <div className="flex flex-col items-center">
            <div className="flex justify-center gap-3">
              <div className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer shadow-sm">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                   <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
              </div>
              {profile?.linkedinUrl && (
                <a href={profile.linkedinUrl} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-[#0A66C2] hover:bg-slate-50 transition-colors shadow-sm">
                  <Linkedin className="w-4 h-4" fill="currentColor" />
                </a>
              )}
              {profile?.twitter && (
                <a href={`https://twitter.com/${profile.twitter.replace('@', '')}`} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-[#1DA1F2] hover:bg-slate-50 transition-colors shadow-sm">
                  <Twitter className="w-4 h-4" fill="currentColor" />
                </a>
              )}
              {profile?.githubUrl && (
                <a href={profile.githubUrl} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-slate-900 hover:bg-slate-50 transition-colors shadow-sm">
                  <Github className="w-4 h-4" fill="currentColor" />
                </a>
              )}
              {profile?.website && (
                <a href={profile.website} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-primary-500 hover:bg-slate-50 transition-colors shadow-sm">
