...
export function ExploreSearch({ value, onChange, placeholder = "Search builders or rooms..." }: ExploreSearchProps) {
  ...
  return (
    <div
      className={`relative flex items-center w-full max-w-md bg-white border rounded-2xl transition-all duration-300 ${
        isFocused
    ...
    >
      ...
      <input
        ...
        className="flex-1 bg-transparent border-none outline-none text-[15px] font-medium text-slate-900 placeholder:text-slate-400 py-3 pr-4"
      />
      {value && (
        <button
          onClick={() => {
        ...
        >
          <div className="w-5 h-5 rounded-full bg-slate-100 flex items-center justify-center group-hover:bg-slate-200 transition-colors">
            <X className="w-3 h-3 text-slate-500 group-hover:text-slate-700" />
          </div>
        </button>
      )}
    </div>
