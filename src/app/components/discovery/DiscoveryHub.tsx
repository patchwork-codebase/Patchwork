  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-100 mb-2">Discovery Projects</h1>
          <p className="text-slate-400 text-sm sm:text-base">Validate assumptions and gather signals before you build.</p>
        </div>
        {!isObserver && (
          <form onSubmit={handleCreate} className="flex gap-2 w-full sm:w-auto">
            <input 
              ...
              className="flex-1 sm:w-64 px-4 py-2 bg-transparent border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-400/50 text-slate-100 placeholder-slate-500 text-sm"
            />
            <button 
              type="submit" 
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 border-2 border-dashed border-slate-800 rounded-3xl bg-transparent">
          <Compass className="w-12 h-12 text-slate-400 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-slate-100 mb-2">No Discovery Projects Yet</h2>
          <p className="text-slate-400 max-w-md mx-auto mb-6">Start a new discovery project to validate your idea with customer interviews, market signals, and clear hypotheses.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects?.map(project => (
            <Link 
              ...
              className="group bg-slate-800/50 border border-slate-700 rounded-2xl p-6 hover:shadow-lg hover:bg-slate-800 hover:border-primary-400/30 transition-all"
            >
              ...
              <h3 className="text-lg font-bold text-slate-100 mb-2 group-hover:text-primary-400 transition-colors line-clamp-1">{project.title}</h3>
