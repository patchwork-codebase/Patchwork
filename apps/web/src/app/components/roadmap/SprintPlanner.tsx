      ...
      <div className="flex-1 w-full space-y-8">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-primary-500 to-indigo-500 hover:from-primary-600 hover:to-indigo-600 shadow-md shadow-primary-500/20 text-slate-900 dark:text-white rounded-xl text-[13px] font-bold transition-all focus-ring active:scale-95"
          >
        ...
        {isCreating && (
          <form onSubmit={handleCreateSprint} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-lg flex flex-wrap sm:flex-nowrap items-center gap-3 relative overflow-hidden">
            ...
            <div className="flex-1 w-full sm:w-auto">
              <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">Sprint Name</label>
              <input
                autoFocus
                type="text"
                ...
                className="w-full text-[14px] font-medium outline-none placeholder:text-slate-600 dark:text-slate-300 bg-transparent"
              />
            </div>
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end mt-2 sm:mt-0">
              ...
              <button type="submit" className="px-4 py-2 bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl text-[12px] font-bold transition-colors shadow-sm">Save Sprint</button>
            </div>
          </form>
        )}
        ...
        <div className="space-y-6">
          {sprints?.length === 0 && !isCreating ? (
            <div className="text-center p-12 border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/50 shadow-sm dark:shadow-none">
              <CalendarIcon className="w-8 h-8 text-slate-600 dark:text-slate-300 mx-auto mb-3" />
              <p className="text-slate-500 text-[14px] font-medium">No sprints created yet.</p>
              <p className="text-slate-500 dark:text-slate-400 text-[13px] mt-1">Sprints help you group tasks for focused development.</p>
            </div>
          ) : (
            sprints?.map(sprint => {
                  ...
                  <div className="p-5 bg-slate-50/50 min-h-[120px]">
                    {sprintItems.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-center py-6">
                        <p className="text-[13px] font-medium text-slate-500 dark:text-slate-400">No items assigned to this sprint.</p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 gap-3">
                        {sprintItems.map(item => (
                          <div key={item.id} className="bg-white p-3.5 rounded-xl border border-slate-100 shadow-sm hover:shadow hover:border-slate-100 flex items-center justify-between group transition-all">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-md overflow-hidden sticky top-[100px]">
          ...
          <div className="p-4 bg-slate-50/50 min-h-[300px] max-h-[600px] overflow-y-auto custom-scrollbar space-y-3">
            {backlogItems.length === 0 ? (
              <p className="text-[13px] font-medium text-slate-500 dark:text-slate-400 text-center py-8">No unassigned items.</p>
            ) : (
              backlogItems.map(item => (
                <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow hover:border-slate-100 transition-all group">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="relative group/menu shrink-0">
                      <button className="p-1 text-slate-500 dark:text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors"><MoreVertical className="w-4 h-4" /></button>
                      <div className="absolute right-0 top-full mt-1 bg-white border border-slate-100 rounded-xl shadow-xl py-1.5 w-48 z-10 hidden group-hover/menu:block origin-top-right animate-in fade-in zoom-in-95 duration-100">
                        <div className="px-3 py-1.5 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Assign to sprint</div>
                        {sprints?.length === 0 ? (
