...
const REACTION_CONFIG: Record<string, { emoji: string; label: string; badge: string }> = {
  ...
  like:        { emoji: '👍', label: 'Like',          badge: 'bg-white/10 text-white border border-slate-300 dark:border-white/20' },
};
...
export default function RoomLogPage() {
  ...
  if (!room) {
    return (
      <div className="max-w-[860px] mx-auto px-6 py-20 text-center">
        <Hammer className="w-12 h-12 mx-auto mb-4 text-slate-600 dark:text-slate-300" />
  ...
  return (
    <div className="max-w-[860px] w-full mx-auto px-4 sm:px-6 py-8 relative">
      ...
      <div className="bg-white border border-slate-100 rounded-[28px] p-6 sm:p-8 mb-6 shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6 relative">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              {room.tags?.[0] && <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest bg-slate-100 text-slate-600 border border-slate-100">{room.tags[0]}</span>}
            ...
            <div className="flex flex-wrap items-center gap-4 text-[12px] text-slate-500 font-medium">
              <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />Started {formatDate(room.createdAt)}</span>
              <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />Built over {days} day{days !== 1 ? 's' : ''}</span>
              {room.builderName && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
        {[
          ...
          <div key={stat.label} className="bg-white border border-slate-100 rounded-[20px] p-4 sm:p-5 shadow-sm flex flex-col gap-2">
      ...
      {(existingRetro || isOwner) && (
        <div className={`border rounded-[20px] p-5 sm:p-6 mb-6 relative overflow-hidden ${existingRetro ? 'bg-gradient-to-br from-slate-900 to-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200 border-dashed'}`}>
          {existingRetro ? (
            <>
              <div className="absolute top-4 right-5 text-[72px] leading-none text-slate-900 dark:text-white/5 font-serif select-none">"</div>
              <div className="flex items-center justify-between mb-3">
                ...
                {isOwner && !retroEditing && (
                  <button onClick={() => { setRetroNote(existingRetro); setRetroEditing(true); }} className="text-[11px] font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">Edit</button>
                )}
              </div>
              {retroEditing ? (
                <>
                  <textarea autoFocus value={retroNote} onChange={e => setRetroNote(e.target.value)} rows={5} className="w-full bg-white/10 border border-slate-300 dark:border-white/20 rounded-xl px-4 py-3 text-[14px] text-slate-900 dark:text-white placeholder-slate-500 resize-none focus:outline-none focus:ring-2 focus:ring-amber-400/50 font-medium leading-relaxed" placeholder="What would you do differently? What worked well? What did you learn?" />
                  <div className="flex gap-2 mt-3 justify-end">
                    <button onClick={() => setRetroEditing(false)} className="px-4 py-2 text-[12px] font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">Cancel</button>
                ...
                <p className="text-[14px] sm:text-[15px] text-slate-700 dark:text-slate-200 font-medium leading-relaxed">{existingRetro}</p>
              )}
            </>
          ) : isOwner ? (
            <>
              ...
              {!retroEditing ? (
                <div className="flex flex-col items-start gap-3">
                  ...
                  <button onClick={() => setRetroEditing(true)} className="px-5 py-2.5 bg-slate-900 text-slate-900 dark:text-white text-[12px] font-bold rounded-full hover:bg-slate-100 dark:bg-slate-800 transition-colors">Write retrospective</button>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex gap-2 mt-3 justify-end">
                    ...
                    <button onClick={saveRetroNote} disabled={retroSaving || !retroNote.trim()} className="px-5 py-2 bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 disabled:opacity-50 text-slate-900 dark:text-white text-[12px] font-bold rounded-full transition-colors">{retroSaving ? 'Saving...' : 'Save retrospective'}</button>
                  </div>
                </>
              )}
            </>
      ...
      {closingNote && (
        <div className="bg-gradient-to-br from-primary-400/5 to-primary-500/5 border border-primary-400/20 rounded-[20px] p-5 sm:p-6 mb-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono ml-auto">{timeAgo(closingNote.createdAt)}</span>
          </div>
      ...
      {topObservers.length > 0 && (
        <div className="bg-white border border-slate-100 rounded-[20px] p-5 sm:p-6 mb-6 shadow-sm">
          ...
          <div className="flex flex-col gap-2">
            {topObservers.map((obs, i) => (
              <div key={obs.id} onClick={() => navigate(`/dashboard/profile/${obs.id}`)} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer group">
                <span className="text-[11px] font-mono font-bold text-slate-600 dark:text-slate-300 w-4">{i + 1}</span>
                ...
                <div className="flex flex-col items-end">
                  ...
                  <span className="text-[9px] text-slate-500 dark:text-slate-400 font-mono mt-0.5">{obs.count} posts</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex gap-1 border-b border-slate-100 mb-6 overflow-x-auto scrollbar-hide">
        {([
      ...
      {activeSection === 'timeline' && (
        <div className="space-y-4">
          {updates.length === 0 ? (
            <div className="text-center py-16 bg-slate-50 border-2 border-dashed border-slate-100 rounded-[24px] shadow-sm dark:shadow-none">
              <TrendingUp className="w-10 h-10 mx-auto mb-3 text-slate-600 dark:text-slate-300" />
              <p className="text-[14px] font-bold text-slate-500 dark:text-slate-400">No updates were posted in this room</p>
            </div>
          ) : (
            <>
                  ...
                  <div key={update.id} className={`w-full max-w-full bg-white border rounded-[20px] p-5 sm:p-6 shadow-sm relative ${isFirst ? 'border-primary-400/30 ring-1 ring-primary-400/10' : 'border-slate-200'}`}>
                    ...
                    <div className="flex flex-col mb-3 gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] text-slate-500 dark:text-slate-400 font-mono ml-auto">{timeAgo(update.createdAt)}</span>
                      </div>
                    </div>
                    <ReadMoreText text={update.content} maxLength={300} className="text-[14px] text-slate-700 leading-relaxed font-medium" />
                    {update.mediaUrl && (
                      <div className="mt-3 w-full max-w-full rounded-xl overflow-hidden border border-slate-100">
              ...
              {updates.length > 5 && (
                <button onClick={() => setShowAllUpdates(prev => !prev)} className="w-full flex items-center justify-center gap-2 py-3.5 border border-slate-100 bg-white hover:bg-slate-50 rounded-[20px] text-[13px] font-bold text-slate-600 hover:text-slate-900 transition-all shadow-sm">
        ...
        return (
          <div className="space-y-4">
            ...
            {textReactions.length > 0 && (
              <div className="bg-white border border-slate-100 rounded-[20px] p-5 shadow-sm mb-6">
          ...
          {aiInsights && aiInsights.themes?.length > 0 && (
            <div className="bg-slate-50 border border-slate-100 rounded-[20px] p-5 sm:p-6 mb-8 relative overflow-hidden shadow-sm dark:shadow-none">
          ...
          <div className="space-y-3">
            {textReactions.length === 0 ? (
            <div className="text-center py-16 bg-slate-50 border-2 border-dashed border-slate-100 rounded-[24px] shadow-sm dark:shadow-none">
              <MessageCircle className="w-10 h-10 mx-auto mb-3 text-slate-600 dark:text-slate-300" />
              <p className="text-[14px] font-bold text-slate-500 dark:text-slate-400">No text reactions were left in this room</p>
            </div>
          ) : (
            textReactions
              .sort((a: any, b: any) => {
                ...
                <div key={r.id} className={`flex items-start gap-4 p-4 sm:p-5 border rounded-[18px] shadow-sm transition-all ${
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      ...
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono ml-auto">{timeAgo(r.createdAt)}</span>
                    </div>
      ...
      {activeSection === 'decisions' && (
        <div className="space-y-3">
          {decisions.length === 0 ? (
            <div className="text-center py-16 bg-slate-50 border-2 border-dashed border-slate-100 rounded-[24px] shadow-sm dark:shadow-none">
              <FileText className="w-10 h-10 mx-auto mb-3 text-slate-600 dark:text-slate-300" />
              <p className="text-[14px] font-bold text-slate-500 dark:text-slate-400">No decisions were logged in this room</p>
            </div>
          ) : (
            decisions.map((d: any) => {
              ...
              return (
                <div key={d.id} className="bg-white border border-slate-100 rounded-[18px] p-5 shadow-sm">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono shrink-0 mt-0.5">{timeAgo(d.created_at || d.createdAt)}</span>
                      </div>
                      {d.description && <p className="text-[13px] text-slate-600 mt-1.5 leading-relaxed font-medium">{d.description}</p>}
                      {d.outcome && (
                        <div className="mt-3 pt-3 border-t border-slate-100">
                          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Outcome · </span>
      ...
      {activeSection === 'milestones' && (
        <div className="space-y-3">
          {milestones.length === 0 ? (
            <div className="text-center py-16 bg-slate-50 border-2 border-dashed border-slate-100 rounded-[24px] shadow-sm dark:shadow-none">
              <Target className="w-10 h-10 mx-auto mb-3 text-slate-600 dark:text-slate-300" />
              <p className="text-[14px] font-bold text-slate-500 dark:text-slate-400">No milestones were tracked in this room</p>
              <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-1">Milestones sync from Linear when connected</p>
            </div>
          ) : (
            milestones.map((m: any) => {
              ...
              return (
                <div key={m.id} className="bg-white border border-slate-100 rounded-[18px] p-5 shadow-sm flex items-start gap-4">
                  <div className={`w-8 h-8 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0 mt-0.5 shadow-sm dark:shadow-none`}>
      ...
      <div className="mt-10 flex items-center justify-between flex-wrap gap-3 py-6 border-t border-slate-100">
        <div className="flex items-center gap-2">
          <RotateCcw className="w-4 h-4 text-slate-500 dark:text-slate-400" />
          <span className="text-[12px] text-slate-500 dark:text-slate-400 font-medium">This room is archived and read-only</span>
        </div>
