...
export function CodeDiffViewer({ data }: CodeDiffViewerProps) {
  ...
  return (
    <div className="my-3 overflow-hidden rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-950 text-slate-700 dark:text-slate-200 text-xs font-mono shadow-md">
      {/* Header Bar */}
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/80 px-3 py-2 text-[11px] text-slate-500 dark:text-slate-400 shadow-sm dark:shadow-none">
        <div className="flex items-center gap-2 font-medium">
          <FileCode className="w-3.5 h-3.5 text-primary-400" />
          <span className="text-slate-700 dark:text-slate-200">{data.filename || "diff_changes.ts"}</span>
          {data.language && (
            <span className="rounded bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 text-[9px] uppercase font-bold text-slate-500 dark:text-slate-400">
              {data.language}
            </span>
          )}
        </div>
