...
export function DecisionLogCard({ roomId, user, reactions = [], queryClient, isNested = false, isBuilder = false }: DecisionLogCardProps) {
  ...
  return (
    <div className={isNested ? "flex flex-col h-full" : "bg-white dark:bg-transparent rounded-[24px] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[500px]"}>
      {!isNested ? (
        <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex flex-wrap sm:flex-nowrap items-center justify-between gap-4 shrink-0">
          <div>
            <h3 className="text-[16px] font-extrabold text-slate-900 dark:text-slate-100 leading-tight flex items-center gap-2">
              Decision log
            </h3>
            <span className="text-[12px] text-slate-500 dark:text-slate-400 font-medium">12 decisions · day 1-12</span>
          </div>
          <div className="flex items-center gap-3 sm:gap-4 shrink-0">
            ...
            <button className="text-[11px] sm:text-[12px] font-bold text-slate-500 hover:text-slate-900 dark:text-white transition-colors uppercase tracking-wider shrink-0 whitespace-nowrap">
              View All
            </button>
          </div>
        </div>
      ) : (
        <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shrink-0 bg-transparent">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] text-slate-500 dark:text-slate-400 font-medium">decisions logged</span>
          </div>
          {isBuilder && (
            <motion.button 
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => setIsModalOpen(true)}
              className="group relative overflow-hidden bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-200 dark:bg-slate-700 border border-slate-100 dark:border-slate-700 text-slate-900 dark:text-white px-4 py-1.5 rounded-full font-bold text-[12px] transition-all flex items-center gap-1.5 shadow-sm"
            >
        ...
        <div className="relative">
          {allDecisions.length === 0 ? (
            ...
            allDecisions.map((decision, index) => {
              ...
              return (
                <div key={decision.id} className={`pb-5 ${index !== allDecisions.length - 1 ? 'border-b border-slate-200 dark:border-slate-800' : ''}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <h4 className="text-[14px] font-bold text-slate-900 dark:text-slate-100 mb-1">{decision.title}</h4>
                        {isBuilder && (
                          <div className="flex items-center gap-1 shrink-0">
                            <button onClick={() => { setEditingDecision(decision); setIsModalOpen(true); }} className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-primary-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 rounded-lg transition-colors border border-slate-300 dark:border-slate-700">
                              <Edit2 className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => setDecisionToDelete(decision.id)} className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-rose-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 rounded-lg transition-colors border border-slate-300 dark:border-slate-700">
                      ...
                      {decision.description && (
                        <p className="text-[13px] text-slate-500 dark:text-slate-400 leading-relaxed mb-2">
                          {decision.description}
                        </p>
                      )}
                      
                      {decision.media_url && (
                        <div className="mb-3 w-fit rounded-xl overflow-hidden border border-slate-100">
                      ...
                      {decision.external_link && (
                        <a 
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-700 rounded-lg text-[12px] font-bold transition-colors mb-3"
                        >
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                        <button 
                          ...
                          className={`flex items-center whitespace-nowrap gap-1.5 px-2.5 py-1 rounded-full text-[12px] font-bold border transition-all ${hasSharp ? 'bg-primary-400/10 text-primary-600 dark:text-primary-400 border-primary-400/30' : 'bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:text-slate-900 dark:hover:text-slate-200'}`}
                        >
                          <span>✦</span> Sharp {sharpCount > 0 && <span className="opacity-70">{sharpCount}</span>}
                        </button>
                        <button 
                          onClick={() => toggleReaction(decision.id, 'pushback')}
                          className={`flex items-center whitespace-nowrap gap-1.5 px-2.5 py-1 rounded-full text-[12px] font-bold border transition-all ${hasPushback ? 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/30' : 'bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:text-slate-900 dark:hover:text-slate-200'}`}
                        >
                        ...
                        <button 
                          ...
                          className="flex items-center gap-1.5 text-[12px] font-bold text-slate-500 hover:text-slate-700 dark:text-slate-200 transition-colors"
                        >
                      ...
                      <AnimatePresence>
                        {itemReplies.length > 0 && (
                          ...
                          >
                            {itemReplies.map((reply: any) => (
                              <div key={reply.id} className="flex items-start gap-3 p-3 bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 rounded-2xl">
                                <UserAvatar 
                                ...
                                <div>
                                  <div className="flex items-center gap-2 mb-0.5">
                                    <span className="text-[12px] font-bold text-slate-900 dark:text-slate-100">Observer</span>
                        ...
                        {replyingTo === decision.id && (
                          <motion.div
                            ...
                            className="mt-4 p-3 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 shadow-sm rounded-2xl relative"
                          >
                            <textarea
                              ...
                              className="w-full bg-transparent border-none focus:ring-0 text-[13px] text-slate-900 dark:text-slate-100 placeholder-slate-500 resize-none h-16 focus-visible:outline-none"
                            />
                            <InlineEmojiPicker
      ...
      <AlertDialog open={!!decisionToDelete} onOpenChange={(open) => !open && setDecisionToDelete(null)}>
        <AlertDialogContent className="bg-[#0D0B14] border border-white/[0.08] text-slate-900 dark:text-white">
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-transparent border-white/[0.08] text-slate-600 dark:text-slate-300 hover:bg-white/[0.05] hover:text-slate-900 dark:text-white">Cancel</AlertDialogCancel>
