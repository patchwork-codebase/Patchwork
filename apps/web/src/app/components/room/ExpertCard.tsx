    ...
    <div 
      className={`relative rounded-2xl p-5 border transition-all duration-300 ${
        selected 
          ...
          : 'bg-white dark:bg-transparent border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:border-primary-500/50 hover:shadow-sm'
      } cursor-pointer`}
    ...
    >
      {selected && (
        <div className="absolute top-4 right-4 h-6 w-6 rounded-full bg-primary-500 flex items-center justify-center shadow-sm">
          <BadgeCheck className="w-4 h-4 text-slate-900 dark:text-white" />
        </div>
      )}
      ...
      <div className="flex items-start gap-4 mb-4">
        <div className="relative">
          {expert.avatar ? (
            <UserAvatar userId={expert.id} name={expert.name} avatarUrl={expert.avatar} className="w-14 h-14 rounded-full object-cover border border-slate-300 dark:border-slate-700" />
          ) : (
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-indigo-500 to-primary-600 flex items-center justify-center text-slate-900 dark:text-white font-bold text-xl shadow-sm">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {expert.domains.map(domain => (
          <span key={domain} className="px-2.5 py-1 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-600 dark:text-slate-300 shadow-sm dark:shadow-none">
            {domain}
          </span>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <div className="bg-slate-50 dark:bg-slate-800/30 rounded-xl p-3 border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none">
          <div className="flex items-center gap-1 text-amber-500 mb-1">
            <Star className="w-4 h-4 fill-current" />
            <span className="font-bold text-sm text-slate-900 dark:text-slate-100">{expert.rating.toFixed(1)}</span>
          </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/30 rounded-xl p-3 border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none">
          <div className="flex items-center gap-1 text-emerald-500 mb-1">
            ...
            <span className="font-bold text-sm text-slate-900 dark:text-slate-100">{expert.typicalResponseTime}</span>
          </div>
      ...
      <div className="flex items-center justify-between mt-auto">
        <div className="flex flex-col">
          <span className="text-[11px] uppercase tracking-wider font-bold text-slate-500 dark:text-slate-400 mb-0.5">Capacity</span>
          ...
          className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
            selected 
              ...
              : isUnavailable 
                ...
                : 'bg-slate-900 dark:bg-slate-800 text-white dark:text-slate-300 hover:bg-slate-800 dark:hover:bg-slate-700 hover:text-slate-100'
          }`}
