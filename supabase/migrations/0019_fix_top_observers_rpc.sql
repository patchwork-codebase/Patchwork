-- Migration: 0019_fix_top_observers_rpc.sql
-- Description: Fix RPC function to calculate top observers by using the correct users table
...
BEGIN
  ...
  FROM public.users u
  LEFT JOIN observer_rooms or_rooms ON u.id = or_rooms.observer_id
  LEFT JOIN observer_reactions or_reactions ON u.id = or_reactions.observer_id
